...
public class PP2Problema18 {
    ...
    public static int minimoMultiple(int a, int b,int c){
        ...
        for (int i=2;i<m;i++){
            while (a%i==0||b%i==0||c%i==0){//busca valores que sean factores de alguno de los numeros
                if (a%i==0)a=a/i;//reduce el valor por el factor si es factor
                if (b%i==0)b=b/i;//reduce el valor por el factor si es factor
                if (c%i==0)c=c/i;//reduce el valor por el factor si es factor
